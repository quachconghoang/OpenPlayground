...
std::list<SearchNode> PathFinder::findPathReversed(World& world, Point3DInt& start, Point3DInt& end)
{
	...
	while (openList.size())	{
		...
		for (int i = 0; i < surrounding.size(); i++) {
			...
			int brWorldIdx = neighborPoint.x() + (neighborPoint.y() + neighborPoint.z() * sy) * sx;
			...
			if (world.positionIsFree(neighborPoint) && brWorld[brWorldIdx] == false) {
				...
				SearchNode node = SearchNode(&Point3DInt(neighborPoint.x(), neighborPoint.y(), neighborPoint.z()), cost, pathCost, current);
